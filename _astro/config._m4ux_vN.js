...
pio:
    ...
    dialog:
        ...
        # 关于链接
        link: "https://nav.kungal.org"`,v=Nr.load(Mr),Dr={Home:T.Home,Archive:T.Archive,Projects:T.Projects,Skills:T.Skills,Timeline:T.Timeline,Diary:T.Diary,Albums:T.Albums,Anime:T.Anime,About:T.About,Friends:T.Friends},he=n=>{if(typeof n=="string"){const i=Dr[n];if(i===void 0)throw new Error(`Unknown LinkPreset: ${n}`);return i}if(typeof n=="number")return n;const e=n.children?.map(he);return e?{...n,children:e}:n},Rr=n=>n.map(he);({...v.post,comment:v.post.comment.twikoo?{...v.post.comment,twikoo:{...v.post.comment.twikoo,lang:v.post.comment.twikoo.lang??v.site.lang}}:v.post.comment});const Br=v.site;v.umami.enabled,v.umami.apiKey,v.umami.baseUrl,v.umami.scripts;Rr(v.navbar.links);const Yr=v.sidebar;v.profile;v.announcement;v.footer;const Hr=v.particle,jr=v.musicPlayer,Pr=v.pio;export{T as L,Yr as a,Hr as b,jr as m,Pr as p,Br as s};
